"""
=============================================================================
Task 3 Part B — Qiskit QAOA on Tiny Scenario (Three-Way Comparison)
=============================================================================
Based on:
...
-------
  ...
  2. D-Wave Neal   - quantum-annealing simulator (D-Wave's official lib)
  3. Qiskit QAOA   - Quantum Approximate Optimization Algorithm
...
class TinyScenario:
    ...
    With T=2 time steps, the UAV needs exactly 1 move to reach the
    target (Manhattan distance = 2 — diagonal move would be needed,
    but moves are 4-connected, so target cannot be reached in one
    step; UAV will get as close as possible).

    Total binary variables = T × num_cells = 2 × 4 = 8
    ...
    Why so small?
      ...
      So n=8 is the practical limit for an interactive benchmark.
...
def dwave_neal(hubo: HUBO, num_vars: int,
               ...
               seed: int = 42) -> dict:
    """Run D-Wave Neal — quantum annealing simulator."""
...
def plot_three_way_trajectories(scenario, results, builder, path):
    ...
    plt.suptitle("Task 3 — Three-Way Solver Comparison (2×2 grid, T=2, 8 variables)",
                 fontsize=14, y=1.02)
    plt.tight_layout()
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "benchmark_report.md"), "w") as f:
        ...
        f.write(f"Same problem solved by three different solvers for a fair benchmark.\n\n")
        ...
        f.write("- **D-Wave Neal**: same algorithm D-Wave QPUs use internally, just on a CPU. ")
        ...
        f.write("problems and could be deployed via D-Wave's real QPU.\n\n")
