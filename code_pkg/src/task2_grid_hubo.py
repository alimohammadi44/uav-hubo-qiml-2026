...
Binary variable:  x[t, v] ∈ {0, 1}
  ...
  Total variables: horizon × |V| = 14 × 64 = 896
...
class Scenario:
    ...
    Start (0,0) → Target (7,7), 6 obstacles, 14-step horizon.
    """
    grid_size : int = 8
    horizon   : int = 20       # Fix 1: Manhattan dist=14, min path=14 moves
                               # horizon=15 gives t=0..14 = 14 moves ✓
...
def trajectory_sa(
    ...
    H_obs   : infeasible cells are excluded from candidates (or penalised).
    ...
    for sweep in range(num_sweeps):
        ...
        for t in rng.permutation(T):
            ...
            # Publication fix:
            # The trajectory-level SA should not propose obstacle cells.
            # Otherwise obstacle avoidance is not guaranteed by construction.
