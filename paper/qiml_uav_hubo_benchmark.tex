...
\end{equation}
This mapping makes the formulation compatible with Qiskit-style QAOA on reduced instances. It does not make the full $8\times8$, $L=20$ case immediately executable: a direct state-variable mapping would require $|V|L=1,280$ qubits before any auxiliary variables. The accompanying code therefore includes a general HUBO/QUBO-to-Pauli expansion that can generate a \texttt{SparsePauliOp}-compatible representation. We treat this mapping as a mathematical and software compatibility path, not as a completed large-scale IBM hardware result.
