"""
=============================================================================
Task 3 (POLISHED v3) — Full Matched 50 vs 50 Benchmark, Proper Checks
=============================================================================
This is the FINAL polished version of Task 3 Tier 1.

WHAT'S DIFFERENT FROM v1 AND v2
-------------------------------
  v1 had:
    - 50 Neal reads vs 50 SA runs (matched)
    - But buggy feasibility check (hard_penalty < 0.5 trivially passed)
    - Reported false 100% feasibility for both solvers

  v2 had:
    - Fixed feasibility check (proper constraint-by-constraint check)
    - But only 10 SA runs (vs 50 Neal reads) — asymmetric
    - Used as a quick fix to avoid re-running SA

  v3 (this file):
    - Fixed feasibility check (same as v2)
    - 50 SA runs (matched to Neal's 50 reads)
    - No caveats needed - clean methodology

WHAT'S IN THIS SCRIPT
---------------------
  1. Build HUBO with hardened weights (λ=1000)
  2. Run 50 D-Wave Neal reads with PROPER feasibility check
  3. Run 50 Classical SA runs with PROPER feasibility check
  4. Decompose every sample into hard-penalty and soft-cost
  5. Run penalty-weight sweep across 6 weight values
  6. Save all plots and JSON

EXPECTED RUNTIME
----------------
  Neal:    ~25 seconds  (50 reads × 3000 sweeps)
  SA:      ~40 minutes  (50 runs × 200 sweeps)
  Sweep:   ~3 minutes
  TOTAL:   ~45 minutes

REQUIREMENTS
------------
  - task2_grid_hubo.py in same folder as this file
  - pip install dwave-neal dimod numpy matplotlib

HOW TO RUN
----------
  cd <your-folder>
  python task3_polished_v3.py

OUTPUTS  →  ./outputs/task3_polished_v3/
=============================================================================
"""
...
OUT_DIR = os.path.join(TASK2_DIR, "outputs", "task3_polished_v3")
...
# ===========================================================================
# 1. HELPERS — CORRECT feasibility check
# ===========================================================================
...
def proper_feasibility_check(bits, scenario, grid) -> dict:
    """
    Independent constraint-by-constraint check.
    Does NOT rely on energy heuristics.
    """
    T, V = grid.T, grid.V
...
def decompose_energy(scenario, grid, bits) -> dict:
    """Split energy into hard-penalty and soft-cost components."""
    H_uniq     = grid.H_uniq()
...
def solve_with_sa_proper(scenario, grid, hubo, num_runs=50, num_sweeps=200):
    """
    Full 50 SA runs with proper feasibility check.
    Matched to Neal's 50 reads for fair comparison.
    """
    samples = []
    t0 = time.perf_counter()
    ...
    for run in range(num_runs):
        ...
        if (run + 1) % 5 == 0:
            ...
            print(f"  SA {run+1}/{num_runs} done, "
                  f"best so far = {min(s['total_energy'] for s in samples):.0f}, "
...
def run_penalty_sweep_proper(weights_to_try, num_reads=15, num_sweeps=2000):
    """Sweep penalty weights with proper feasibility checking."""
    print(f"\n{'─' * 70}")
...
def plot_total_energy(neal_samples, sa_samples, path):
    ...
    ax.set_ylabel("Count")
    ax.set_title("Total Energy Distribution  (50 vs 50 matched samples)")
...
def plot_split_energy(neal_samples, sa_samples, path):
    """Hard-penalty and soft-cost histograms side-by-side."""
...
def plot_runtime(neal_rt, sa_rt, neal_n, sa_n, path):
    ...
    ax.set_title("Runtime — 50 vs 50 matched samples")
...
def main():
    print("=" * 70)
    print("  Task 3 (POLISHED v3) — Full Matched 50 vs 50 Benchmark")
    print("=" * 70)
    print("\nClean methodology: matched sample sizes (50 reads vs 50 runs),")
    print("proper feasibility check, full penalty sweep. No caveats needed.")
    print("\nExpected runtime: ~45 minutes (mostly the 50 SA runs).")
    ...
    # Find Neal's best feasible sample
    ...
    # ---- Classical SA ----
    print(f"\n{'─' * 70}")
    print("Classical SA — 50 runs (matched), proper feasibility check")
    print(f"{'─' * 70}")
    print("This is the long part — expected ~40 minutes.")
    print("Progress will be reported every 5 runs.")
    sa_result = solve_with_sa_proper(scenario, grid, hubo,
                                      num_runs=50, num_sweeps=200)
    ...
    plot_trajectory(scenario, sa_samples[0]["bits"], grid,
                     f"Classical SA — Best Trajectory\n"
    ...
    json_payload = {
        ...
        "methodology": (
            "v3: matched 50 vs 50 samples, proper feasibility check, "
            "full penalty sweep. No methodological caveats."
        ),
        "d_wave_neal": {
            "num_reads": 50,
    ...
    with open(os.path.join(OUT_DIR, "benchmark_results_v3.json"), "w") as f:
        json.dump(json_payload, f, indent=2)
    print(f"  Saved: benchmark_results_v3.json")

    # ---- Final summary ----
    print(f"\n{'=' * 70}")
    print(f"  TASK 3 POLISHED v3 — FINAL RESULTS  (50 vs 50 matched)")
    print(f"{'=' * 70}")
    print(f"  D-Wave Neal:")
    print(f"    Best total energy : {neal_samples[0]['total_energy']:.2f}")
    print(f"    Best soft cost    : {neal_samples[0]['soft_cost']:.2f}")
    ...
    if neal_best_feasible['feasible']:
        print(f"    Best FEASIBLE soft: {neal_best_feasible['soft_cost']:.2f}")
    print(f"  Classical SA:")
    print(f"    Best total energy : {sa_samples[0]['total_energy']:.2f}")
    print(f"    Best soft cost    : {sa_samples[0]['soft_cost']:.2f}")
