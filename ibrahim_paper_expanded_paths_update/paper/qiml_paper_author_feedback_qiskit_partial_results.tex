...
\subsection{IBM/Qiskit Hamiltonian Compatibility}
...
\begin{equation}
    x_i=\frac{\mathbb{I}-Z_i}{2},
\end{equation}
where $Z_i$ is the Pauli-$Z$ operator on qubit $i$. A general HUBO monomial maps as
\begin{equation}
    c\prod_{i\in S}x_i \mapsto \frac{c}{2^{|S|}}\sum_{R\subseteq S}(-1)^{|R|}\prod_{i\in R}Z_i .
    \label{eq:pauli_mapping}
\end{equation}
...
\begin{equation}
\hat{H}_C=c_0+\sum_i h_iZ_i+\sum_{i<j}J_{ij}Z_iZ_j+
\sum_{i<j<k}J_{ijk}Z_iZ_jZ_k .
\end{equation}
This mapping makes the formulation compatible with Qiskit-style QAOA on reduced instances. It does not make the full $8\times8$, $L=20$ case immediately executable: a direct state-variable mapping would require $|V|L=1,280$ qubits before any auxiliary variables. The accompanying code therefore includes two Qiskit paths: (i) a general HUBO/QUBO-to-Pauli expansion that can generate a \texttt{SparsePauliOp}-compatible representation, and (ii) a reduced $2\times2$, $L=3$ QUBO/QAOA sanity check using Qiskit Optimization. In the current author-feedback run, the Hamiltonian mapping completed successfully, while the $p=1$ QAOA optimizer was interrupted before returning a sample. Therefore the Qiskit experiment is treated as a Hamiltonian-compatibility and implementation-status check, not as a scalable performance result.
...
\subsection{Penalty Sweep and Reduced Qiskit Check}
...
A reduced $2	imes2$, $L=3$ Qiskit-compatible test is included only as a software and Hamiltonian-compatibility check. This reduced problem has 12 binary variables, corresponding to 12 qubits under direct encoding, and 38 QUBO terms. The HUBO/QUBO-to-Pauli script completed successfully and generated a Hamiltonian with 39 Pauli terms including the identity offset. In the same reduced run, classical bit-flip SA and 	exttt{neal} found the same feasible target-reaching solution with energy $-195.4477$. A $p=1$ Qiskit QAOA/SPSA state-vector run was initialized on the 12-qubit instance, but the run was manually interrupted during SPSA calibration before any QAOA sample was returned. We therefore do not report a QAOA energy or trajectory in the present table. This is an implementation-status result rather than a performance result; it still documents the complete Hamiltonian mapping path and identifies the remaining QAOA-runtime item to be completed before final submission.

\begin{table}[t]
\centering
\caption{Reduced IBM/Qiskit compatibility check from the current author-feedback run.}
\label{tab:qiskit_reduced_status}
\scriptsize
\begin{tabular}{@{}ll@{}}
\toprule
Quantity & Value\\
\midrule
Reduced grid and horizon & $2\times2$, $L=3$\\
Binary variables / direct qubits & 12\\
QUBO terms & 38\\
Linear / quadratic terms & 12 / 26\\
Pauli terms including identity & 39\\
Identity coefficient & 276.1569\\
Classical SA best energy & $-195.4477$\\
Classical SA feasibility & full success\\
\texttt{neal} best / mean energy & $-195.4477$ / $-195.4477$\\
\texttt{neal} feasibility & full success\\
Qiskit QAOA status & initialized, interrupted before sample\\
\bottomrule
\end{tabular}
\end{table}
