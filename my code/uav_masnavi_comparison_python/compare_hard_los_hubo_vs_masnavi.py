...
def build_scenario(name: str) -> Scenario:
    ...
    if name == "visible_start":
        # Same map/target, but start is moved to a visible cell for a valid hard-LOS comparison.
        return Scenario(start=(3, 0), target=(7, 7), horizon_points=12)
...
def trajectory_metrics(s: Scenario, points: np.ndarray, runtime_s: float, name: str) -> Dict[str, object]:
    ...
    return {
        ...
        "hard_feasible": bool(visible.all() and collision_free.all() and start_error <= 1e-6 and terminal_error <= 1e-6),
    }
...
def masnavi_style_continuous_optimize(
    ...
    u_values = np.linspace(0.0, 1.0, los_samples, endpoint=False)  # include u=0 collision, exclude target u=1
    ...
    def constraint_margins(z: np.ndarray) -> np.ndarray:
        ...
        for p in pts:
            for u in u_values:
                los_p = (1.0 - u) * p + u * target
                diff = los_p[None, :] - centers
                val = np.sum(diff * diff, axis=1) / (r * r) - 1.0
                margins.extend(val.tolist())
...
def main() -> None:
    ...
    cont = masnavi_style_continuous_optimize(s, astar_pts, los_samples=9, maxiter=800)
